...
answer to different closures: the AI share of wealth rises in every demand
closure we study, while whether output grows or winds down during the episode
is decided by how much of the AI surplus recycles as demand --- so a
...
can eradicate capital at an output cost, while an ownership diversion splits
the same accumulation between private and public hands at no output cost by
construction. We state every assumption with its type (anchored to
...
Upkeep on private stock $mK_i$        &     & $+$ & $-$ &     \\
Upkeep on public stock $mK^{\mathrm{pub}}$ & & $+$ &  & $-$ \\
...
depreciation). The public fund does \emph{not} reinvest its own earnings:
...
$r$ & recycled surplus share & arbitrary-but-swept & the closure family of Prop.~\ref{prop:decouple}; E2 axis\\
...
for $e \in (m/v,\, e^{*})$ profit stays positive and decay is geometric at
rate $1-\delta+s(ev-m) < 1$; for $e \le m/v$ profit is non-positive, the
loss is charged to the stock, and capital is eliminated without ever earning
its upkeep.
\end{proposition}
...
With no AI systems active, total money
$M_t = \sum_h y_{h,t} + \sum_h W_{h,t}$ is invariant under the household
spend/save/drawdown dynamics, and the economy has a line of neutral fixed
points indexed by $M_0$, along which aggregate wealth and aggregate income
satisfy $\sum_h W_{h}^{*} \,/\, \sum_h y_{h}^{*} = \sigma_s/\sigma_d$.
\end{proposition}
...
$r \in [0,1]$ of the AI surplus (upkeep, investment, and consumption
returned as sector demand; $r=1$ is the closure of
Section~\ref{sec:substrates}, $r<1$ hoards the remainder as in
\texttt{io\_economy}'s low-reinvestment endpoint). Then: (i) the AI share of
wealth is increasing along the episode for \emph{every} $r$, because
$\Delta(W_A + K)$ is positive whenever $\pi > 0$ regardless of where the
spending lands; (ii) whether aggregate output grows or winds down is decided
by $r$: at $r = 1$ total final demand is conserved up to the household
wealth buffer and output is non-decreasing, while for $r$ below a closure
threshold the hoard is a demand leak and output contracts toward the
AI-self-demand floor. Concentration is the robust event; output is the
closure-dependent one.
\end{proposition}
...
investment-demand channel. An ownership diversion $\omega$ leaves the
aggregate capital dynamics unchanged and is output-neutral by construction;
it changes only the ownership split of the same accumulation, with
$K^{\mathrm{pub}}/K^{\mathrm{tot}} \to \omega$ in the compounding regime.
\end{proposition}
\noindent\emph{Proof sketch.} For the tax: $\relu{\pi}$ enters
\eqref{eq:kupdate} only through $s(1-\tau)\relu{\pi}$, so the small-$K$
growth factor is $1-\delta+s(1-\tau)(ev-m)$ and the threshold algebra of
Proposition~\ref{prop:knee} applies with $s \mapsto s(1-\tau)$. For the
diversion: private and public stocks in a sector earn and pay at identical
per-unit rates ($\mathrm{rev} \propto K$, upkeep $= mK$), so
$K^{\mathrm{tot}} = K_i + K^{\mathrm{pub}}$ obeys the \emph{same} update as
the undiverted stock and the split evolves as a linear share converging to
$\omega$. \emph{This same-per-unit-rates condition is load-bearing}: fixed
costs, scale economies, or differential public-fund efficiency would break
the additivity, and the output-neutrality claim with it.
...
never does. The $\tau$ half is a genuine empirical claim (no closed form for
the output effect); the $\omega$ half follows from
Proposition~\ref{prop:defenses} and its sweep in E3 is an
implementation-fidelity check, not new economics.
\end{corollary}
